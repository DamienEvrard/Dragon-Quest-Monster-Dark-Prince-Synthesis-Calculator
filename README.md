# Calculateur de synthese - DQM: The Dark Prince 

## TBD
- A revoir calcule du niveau minimum des parents de la synthèse pour débloquer un nouveau talent (hors talents "Boost" ou "Afficionado")
- Retirer la traduction française des talents & skills
- Liste des monstres dressables triés par régions
- Affiche la liste avec compte de chaque monstres (feuilles) à obtenir avec caracteristiques (talents) 
- Possibilité pour chaque monstre de l'arbre de le substituer par un autre monstre éligible, la suite de l'arbre à partir de ce noeux est recalculé

---

Interface web (Flask).

## Installation

```
pip install -r requirements.txt
```

## Lancer l'application

```
python3 app.py
```
**http://127.0.0.1:5000**

